...
<head>
    ...
    <style>
        ...
        .belum-diverifikasi { background-color: #fff3cd; color: #856404; }
        .sudah-diverifikasi { background-color: #d4edda; color: #155724; }
        .batal-verifikasi { background-color: #f8d7da; color: #721c24; }
        .belum-diproses { background-color: #e2e3e5; color: #383d41; }
        .dikemas { background-color: #cce5ff; color: #004085; }
        .dikirim { background-color: #d1ecf1; color: #0c5460; }
        ...
        .aksi-btn button.lihat { background-color: #007bff; color: white; }
        .aksi-btn button.hapus { background-color: #dc3545; color: white; }
                ...
                <tr>
                    ...
                    <td>
                        <select class="status-select <?= strtolower(str_replace(' ', '-', $pesanan['status_pengiriman'])) ?>" 
                            ...
                            <option value="Batal" <?= $pesanan['status_pengiriman'] == 'Batal' ? 'selected' : '' ?>>Batal</option>
                        </select>
                    ...
                    <td class="aksi-btn">
                        ...
                        <form action="hapus_pesanan.php" method="POST" onsubmit="return confirm('Yakin ingin menghapus pesanan ini?')">
                            <input type="hidden" name="id_pesanan" value="<?= $pesanan['id_pesanan'] ?>">
    ...
    select.addEventListener('change', function () {
        ...
        .then(result => {
            if (result === 'success') {
                ...
                const shippingLoader = shippingSelect.nextElementSibling;

                if (newStatus === 'Sudah Diverifikasi') {
                    shippingSelect.disabled = false;
                } else {
                    shippingSelect.disabled = true;
                    const defaultStatus = newStatus === 'Batal Verifikasi' ? 'Batal' : 'Belum Diproses';
                    shippingSelect.value = defaultStatus;
                    shippingSelect.className = 'status-select ' + defaultStatus.toLowerCase().replace(' ', '-');
                    shippingLoader.style.display = 'inline-block';

                    fetch('update_status_pengiriman.php', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                        body: `id_pesanan=${idPesanan}&status_pengiriman=${encodeURIComponent(defaultStatus)}`
                    }).finally(() => shippingLoader.style.display = 'none');
                }
            } else {
